...
export function groundClaim(source: string, claim: SemanticEvidenceClaim, signal: Pick<ObservableSignalV2, "dimensionId" | "id" | "verification">): GroundedClaim | EvidenceFinding {
  ...
  if (signal.verification.kind === "distribution") {
    ...
    if (pacingMetrics) {
      ...
      if (distinct.size < 3 || !(goal < pressure && pressure <= beat && beat <= turn) || regionFor(source, anchors[goal], policy) !== "opening" || regionFor(source, anchors[turn], policy) !== "ending") return { ruleId: "evidence.distribution_insufficient", severity: "rewrite", dimensionId: signal.dimensionId };
