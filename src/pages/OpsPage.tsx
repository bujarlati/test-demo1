...
import type { GenerationJob, OpsMetrics } from "../types";
...
export function OpsPage() {
  ...
  const load = async () => {
    try {
      ...
      setMetrics(payload.metrics); setJobs(payload.jobs); setError(null);
        ...
        <section className="jobs-panel">
          ...
          <div className="table-wrap">
            <table><thead><tr><th>故事 / 章节</th><th>任务</th><th>模型</th><th>Token</th><th>延迟</th><th>成本</th><th>状态</th></tr></thead>
              <tbody>{jobs.map((job) => <tr key={job.id}><td><strong>{job.storyTitle}</strong><small>第 {job.chapterNumber} 章 · {formatDateTime(job.createdAt)}</small></td><td>{job.task === "chapter" ? "续章" : job.task === "retcon" ? "修史" : "状态提取"}</td><td>{job.model}</td><td>{job.tokens.toLocaleString("zh-CN")}</td><td>{(job.latencyMs / 1000).toFixed(1)}s</td><td>{money(job.cost)}</td><td><span className={`job-status job-status--${job.status}`}>{job.status === "completed" ? "完成" : job.status === "running" ? "运行中" : "失败"}</span></td></tr>)}</tbody>
            </table>
          </div>
        </section>
